...
AUTH_PROTECTION_BACKEND = getattr(
    settings,
    'AUTH_PROTECTION_BACKEND',
    'django.contrib.auth.backends.ModelBackend'
)

AUTH_PROTECTION_CLASS = str(AUTH_PROTECTION_BACKEND.split('.')[-1])
AUTH_PROTECTION_MODULE = str(('.').join(AUTH_PROTECTION_BACKEND.split('.')[:len(AUTH_PROTECTION_BACKEND.split('.'))-1]))
